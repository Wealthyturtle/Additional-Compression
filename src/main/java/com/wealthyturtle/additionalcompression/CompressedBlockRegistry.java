...
public class CompressedBlockRegistry {
	...
	public static void registerCompressableBlock(String name, Item base, int meta, int max) {
		...
		compressedBlocks.add(new CompressedInfos(name, compressedBlock, base, meta, max));
	}
	...
	public static void init() {
		for (CompressedInfos block : compressedBlocks) {
			...
			GameRegistry.addRecipe(new ShapedOreRecipe(new ItemStack(block.baseItem, 9, block.baseMeta), "X", 'X', blockName));
	...
	public static class CompressedInfos {
		...
		public CompressedInfos(String name, Block block, Item base, int meta, int max) {
			compressedName = name;
